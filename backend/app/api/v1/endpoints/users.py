...
from app.api.v1.endpoints.api_models import UserSettingsResponse, UpdateUserSettingsRequest, BaseResponse
...
def get_current_user(Authorize: AuthJWT = Depends(AuthJWT), session: Session = Depends(get_session)) -> User:
    ...
    return user
@router.get("/{user_id}/preferences/", response_model=BaseResponse[UserSettingsResponse])
async def get_user_preferences(
    ...
    return BaseResponse[UserSettingsResponse](data=user_preferences)
